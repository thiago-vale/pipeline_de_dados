...
def run_etl():
    ...
    try:
        ...
        # Carregar dados
        df_sales = spark.read.format('parquet').load('s3a://datalake-test-thiago/01-bronze/spark/train')
        df_store = spark.read.format('parquet').load('s3a://datalake-test-thiago/01-bronze/spark/store')
